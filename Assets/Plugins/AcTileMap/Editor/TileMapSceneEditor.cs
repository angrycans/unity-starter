...
namespace ACANS.TileMap
{
  partial class TileMapEditor : Editor
  {
    ...
    private void ToolbarWindow(int id)
    {
      ...
      GUI.Label(secondaryTileRect, text);
      ...
      GUI.Label(primaryTileRect, text);
      GUI.contentColor = Color.white;
      ...
      if (tileMap.selectedScriptableTool >= 0 && tileMap.selectedScriptableTool < tileMap.scriptableToolCache.Count)
      {
        ...
        if (fields.Length > 0)
        {
          GUILayout.Label("Settings", GUILayout.Width(100));
        ...
        for (int y = 0; y < rows; y++)
        {
          ...
          for (int x = 0; x < columns; x++)
          {
            ...
            if (index < 0)
            {
              ...
              GUI.Label(GUILayoutUtility.GetLastRect(), "None");
            }
    ...
    private void HandleMouseEvents()
    {
      ...
      if (tileMap.selectedScriptableTool >= 0 && tileMap.selectedScriptableTool < tileMap.scriptableToolCache.Count)
      {
        if (e.button == 0)
        {
          ...
          Handles.Label((Vector2)cursorPoint, point.ToString());
